          ...
          merchentSelects.map((v,k)=>{
            return <FormItem key={k}>
              {getFieldDecorator(v.key, {
                ...
                <SelectComs placeholder="请选择"  labelName={v.labelName} style={{ width: 120 }} >
                  {
                    v.optionVal.map((i,j)=>{
             ...
             return (
               <FormItem key={k}>
                 {getFieldDecorator(v.key, {
                   ...
                   <Input placeholder={"请输入"} addonBefore={(<span style={{minWidth:'70px',display: 'inline-block'}}>{v.labelName}</span>)}  style={{width:'200px',margin:'10px'}}/>
                 )
                 }
               </FormItem>
             )
